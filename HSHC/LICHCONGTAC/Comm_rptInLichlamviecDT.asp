...
set session("oRssubAH") = Server.CreateObject("ADODB.Recordset")
set session("oRssubP") = Server.CreateObject("ADODB.Recordset")
set session("oRssubPH") = Server.CreateObject("ADODB.Recordset")

session("oRs").ActiveConnection = oConn
session("oRssubA").ActiveConnection = oConn
session("oRssubAH").ActiveConnection = oConn
session("oRssubP").ActiveConnection = oConn
session("oRssubPH").ActiveConnection = oConn
	...
	strSQL = "CreateDateWeek " &_
		"@FromDay	  = '"& WeekFrom &"'"	
	strsubSQL = "Rpt_Lichlamviec " &_
		"@FromDay	= '"& WeekFrom & "'," &_
		"@Hour		= ''," &_
		"@Location	= '" & strLocation &"'," &_
		"@ChairMan	= '" & strChairMan &"'," &_
		"@DK		= '" & strDK & "'," &_
		"@Type		= 'DT'"
		

	
	'Response.Write strsubSQL & "<br>"
	'Response.Write strSQL
	'Response.Write "SQLA" & strsubSQLA 
	'response.End 
	
session("oRs").Open strSQL
session("oRssubA").Open strsubSQL

reportname 	= "report\rpt_inlichlamviecTD.rpt"
subreportnameA 	= "rpt_SubPMInLichLamViec"
...
rem strFromday = DateValueVN(WeekFrom)
rem strToday= DateValueVN(DateAdd("d",5,WeekFrom))
strFromday = WeekFrom
strToday= DateValueVN(DateAdd("d",4,strFromday))
