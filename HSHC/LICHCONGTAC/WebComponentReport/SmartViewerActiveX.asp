...
<OBJECT ID="CRViewer"
	...
	WIDTH=100% HEIGHT=95%
	CODEBASE="WebComponentReport/activexviewer.cab#Version=8,0,0,224">
...
<%
	...
	ViTri = InstrRev(strURL,"/")
	Path =  Left(strURL,ViTri)
%>
...
Sub Page_Initialize
	...
	if ScriptEngineMajorVersion < 2 then
		...
		CRViewer.ReportName = Location.Protocol + "//" + Location.Host + "<%=Path%>" + "webcomponentreport/rptserver.asp"
	else
		Dim webSource
		Set webSource = CreateObject("WebReportSource.WebReportSource")	
	
		webSource.ReportSource = webBroker
		webSource.URL = Location.Protocol + "//" + Location.Host + "<%=Path%>" + "webcomponentreport/rptserver.asp" 
