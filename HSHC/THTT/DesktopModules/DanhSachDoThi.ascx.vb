...
#End Region
        ...
        Private Sub LoadGrid()
            ...
            strURL = EditURL("ID", "value" & "&SelectID=" & Request.Params("SelectID") & "&Val=" & RTrim(Request.Params("Val").ToString()) & "&SelectTitle=" & Request.Params("SelectTitle") & "&TuNgay=" & Request.Params("TuNgay").ToString() & "&DenNgay=" & Request.Params("DenNgay").ToString() & "&SelectIndex=" & Request.Params("SelectIndex").ToString() & "&Type=" & Request.Params("Type").ToString() & "&Group=" & Request.Params("Group").ToString() & "&SelectChildIndex=" & Request.Params("SelectChildIndex").ToString() & "&SelectChildID=" & Request.Params("SelectChildID").ToString(), "CHITIETDT")
            ...
            ds = objDanhSach.DanhSachDoThiPhuong_DS(Request.Params("TuNgay").ToString(), Request.Params("DenNgay").ToString(), Request.Params("Val").ToString(), strGroup, Request.Params("Type").ToString(), Request.Params("SelectID").ToString(), ConfigurationSettings.AppSettings("DBCOMMON").ToString(), Request.Params("LoaiCP").ToString(), strURL)

            'BindControl.BindDataGrid(ds, dgdDanhSach, ds.Tables(0).Rows.Count)
            BindControl.BindGrdHoSo(ds, dgdDanhSach, False, "Số GP", "100,100,200,400,100,100", False, True)
            ds = Nothing
            objDanhSach = Nothing
        End Sub
