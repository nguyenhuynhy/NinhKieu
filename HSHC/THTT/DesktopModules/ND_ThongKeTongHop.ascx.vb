...
Namespace THTT
    Public Class ND_ThongKeTongHop
...
#End Region
        ...
        Private Sub LoadGrid()
            Dim objThongKe As New ThongKeNhaDatController
            Dim ds As DataSet
            ds = objThongKe.ThongKeNhaDat_TheoLoaiHoSo(txtTuNgay.Text, txtDenNgay.Text, ConfigurationSettings.AppSettings("COMMONDB").ToString(), CStr(Request.Params("SelectID")))
        ...
        Private Sub dgdDanhSach_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgdDanhSach.PreRender
            ...
            dgitem.Cells.Add(InitCell("Loại hình biến động", 1, 2))
            dgitem.Cells.Add(InitCell("Đã nhận hồ sơ đăng ký", 2, 1))
            dgitem.Cells.Add(InitCell("Đã chỉnh lý hồ sơ địa chính", 2, 1))
            dgdDanhSach.Controls(0).Controls.AddAt(0, dgitem)
            dgitem = Nothing
            'row 2
            dgitem = New DataGridItem(0, 0, ListItemType.Header)
            dgitem.Cells.Add(InitCell("Số trường hợp"))
