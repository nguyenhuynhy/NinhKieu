...
// Compatible Menu Version:  <Min: 1.1.1.0>             //
//                           <Max: 1.1.1.2>             //
// <Script Version: 1113>                               //
//------------------------------------------------------//
var m_oSolpartMenu;
...
__db(this._m_oMenu.id + ' - GenerateMenuHTML');
  ...
  //if (spm_browserType() == 'ie' && spm_isMac() == false)
  ...
  __db(sID + ' getmenuitems');
		  ...
		  if (sParentID == "-1")	//'if top level menu item
		  {
  			...
  			else
  			{
					...
					sHTML += '<td NOWRAP="NOWRAP" ' + sAlign + '>' + spm_getImage(oNode, this) + spm_getItemHTML(oNode, 'left', '&nbsp;') + oNode.getAttribute('title') + spm_getItemHTML(oNode, 'right') + MyIIf(Number(this.rootArrow) && spm_nodeHasChildren(oNode), '</td>\n<td align="right" class="' + spm_fixCSSForMac(this.getIntCSSName('spmrarw') + this.cssMenuRootArrow) + '">' + spm_getArrow(this.rootArrowImage, this) + "", '&nbsp;') + '\n</td>\n</tr>\n</table>\n</td>\n';
				}
  ...
  //--- handles mouseover for menu item ---//
    ...
    //setClassColor(oRow, 'spmitm', this.selForeColor);
        ...
        //oMenu.style.position = "absolute";  //CSS FIX!				
				oMenu.style.left = spm_elementLeft(oRow) + spm_getElementWidth(oRow) - spm_getBodyScrollLeft();
				oMenu.style.top = spm_elementTop(oRow);
				
				this.doTransition(oMenu);

				oMenu.style.display = "";
				if (spm_elementLeft(oRow) - spm_getBodyScrollLeft() + spm_getElementWidth(oRow) + spm_getElementWidth(oMenu) > spm_getViewPortWidth())
				{
					oMenu.style.left = spm_elementLeft(oRow) - spm_getElementWidth(oMenu) - spm_getBodyScrollLeft();
					//oMenu.style.top = spm_elementTop(oRow);					
				}

			  if (spm_elementTop(oMenu) - spm_getBodyScrollTop() + spm_getElementHeight(oMenu) > spm_getViewPortHeight())
			  {
				  oMenu.style.top = spm_getViewPortHeight() + spm_getBodyScrollTop() - spm_getElementHeight(oMenu);
			  }
...
//global
	...
	function spm_hideElement(elmID, eMenu)
	{
		...
		{
			obj = spm_getTags(elmID)[i];
			if (spm_elementTop(obj) > parseInt(eMenu.style.top) + spm_getElementHeight(eMenu))
			{
				//if element is below bottom of menu then do nothing
			}
			else if (spm_elementLeft(obj) > parseInt(eMenu.style.left) + spm_getElementWidth(eMenu))
			{
				//if element is to the right of menu then do nothing
			}
			else if (spm_elementLeft(obj) + spm_getElementWidth(obj) < parseInt(eMenu.style.left))
			{
				//if element is to the left of menu then do nothing
			}
			else if (spm_elementTop(obj) + spm_getElementHeight(obj) < parseInt(eMenu.style.top))
			{
	...
	function spm_positionMenu(me, oMenu, oCell)
	{
		...
		{
			oMenu.style.left = spm_elementLeft(oCell, true) + spm_getElementWidth(oCell) - spm_getBodyScrollLeft();
			oMenu.style.top = spm_elementTop(oCell, true);
			oMenu.style.display = "";
		}
		else
		{
			oMenu.style.left = spm_elementLeft(oCell, true) - spm_getBodyScrollLeft();
			oMenu.style.top = spm_elementTop(oCell, true) + spm_getElementHeight(oCell);
			oMenu.style.display = "";
			
			if (spm_elementLeft(oMenu) - spm_getBodyScrollLeft() + spm_getElementWidth(oMenu) > spm_getViewPortWidth())
			{
			  if (spm_getViewPortWidth() - spm_getElementWidth(oMenu) > 0)  //only do this if it fits
				  oMenu.style.left = spm_getViewPortWidth() - spm_getElementWidth(oMenu);
//				oMenu.style.top = spm_elementTop(oCell) + oCell.offsetHeight;
			}
			
			if (spm_elementTop(oMenu) - spm_getBodyScrollTop() + spm_getElementHeight(oMenu) > spm_getViewPortHeight())
			{
			  if (spm_elementTop(oCell, true) - spm_getElementHeight(oMenu) > 0) //only do this if it fits
				  oMenu.style.top = spm_elementTop(oCell, true) - spm_getElementHeight(oMenu);	//place above menu bar
				//else
 ...
 */     
	...
	function spm_ScrollMenuClick()
	{
		var oCell = event.srcElement;
		var oTbl = oCell.parentElement.parentElement.parentElement;
		var oMenu = oTbl.parentElement;

		if (oCell.id == oMenu.id + 'up')
			oMenu.scrollPos--;
		else 
			oMenu.scrollPos++;
	...
	function showScrolledItems(oMenu)
	{
		var oTbl = oMenu.all[0];
		...
		if (oMenu.scrollPos > 1)
			oTbl.rows[0].style.display = '';
		else
			oTbl.rows[0].style.display = 'none';
		
		
		if (oMenu.scrollPos + oMenu.scrollItems < oTbl.rows.length)
			oTbl.rows[oTbl.rows.length-1].style.display = '';
		else
			oTbl.rows[oTbl.rows.length-1].style.display = 'none';
