...
function FTB_Button(id, commandIdentifier, customAction, customStateQuery, htmlModeEnabled, customEnabled) {
	//alert('id=' + id + '---customAction=' + customAction);
...
FTB_Button.prototype.Click = function() {
	if (!this.disabled) {
		...
		{
			//this.customAction();	
			if(this.customAction!='function() { this.ftb.InsertTableWindow(); }'){
				this.customAction();
			}else{
				this.ftb.InsertTableWindow();
			}
		}
		if (this.commandIdentifier != null && this.commandIdentifier != '') 
		{
...
FTB_FreeTextBox.prototype.LaunchTableWindow = function() {
	alert('a');
}
